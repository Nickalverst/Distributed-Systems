...
def main():
    ...
    interests = ['promocao.jogo', 'promocao.eletronicos']

    result = channel.queue_declare(queue='', exclusive=True)
    queue_name = result.method.queue

    for category in interests:
        channel.queue_bind(exchange=exchange_name, queue=queue_name, routing_key=category)

    print(f"[*] Client 2 waiting for {interests}. To exit press CTRL+C")
    ...
    def callback(ch, method, properties, body):
        ...
        print(f"\n[{status}]")
        print(f"Category: {event.category.capitalize()}")
